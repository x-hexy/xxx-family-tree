...
export function TopBar({ readOnly = false }: TopBarProps) {
  ...
  const findAndSelectMemberByName = useFamilyStore(
    (s) => s.findAndSelectMemberByName,
  );
  ...
  return (
    <>
      <header className="relative flex h-[90px] items-center justify-between border-b border-bronze/40 bg-parchment/90 px-5 backdrop-blur">
        ...
        <div>
          <h1 className="font-serifCn text-xl tracking-[0.16em] text-ink">
            X氏家谱
          </h1>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end gap-1">
            <div className="flex items-center gap-1 rounded border border-bronze/35 bg-[#f8f1e0]/80 p-1">
              ...
              />
              <ModeButton
                active={viewMode === "paternal"}
                label="父系"
                onClick={() => setViewMode("paternal")}
              />
              <ModeButton
                active={viewMode === "maternal"}
                label="母系"
            ...
            <div className="flex items-center gap-2 text-xs text-soot">
              <label className="flex items-center gap-1">
                <input
                  ...
                  onChange={(event) =>
                    setLineVisibility({
                      showParentChildLines: event.target.checked,
                    })
                  }
          ...
          {readOnly ? (
            <button className="rounded border border-bronze/50 px-3 py-1 text-sm text-soot">
              图例
            </button>
          ) : (
            <>
