...
export async function loadUnitSnapshot(treeId: string): Promise<UnitSnapshot> {
  ...
  const [
    ...
    supabase
      .from("members")
      .select("id,name,title,birth_year,generation,bio,avatar_url")
      .eq("tree_id", treeId),
    supabase
      .from("family_units")
      .select("id,name,generation,layout_x,layout_y")
      .eq("tree_id", treeId),
    supabase.from("family_unit_members").select("unit_id,member_id,role"),
    supabase
      .from("unit_relations")
      .select("id,from_unit_id,to_unit_id,relation_type"),
    supabase
      .from("share_settings")
  ...
  return {
    members: (membersResult.data ?? []).map((row: unknown) =>
      mapMember(row as MemberRow),
    ),
    units: (unitsResult.data ?? []).map((row: unknown) =>
      mapUnit(row as UnitRow),
    ),
    unitMembers: (unitMembersResult.data ?? []).map((row: unknown) =>
      mapUnitMember(row as UnitMemberRow),
    ),
    unitRelations: (unitRelationsResult.data ?? []).map((row: unknown) =>
      mapUnitRelation(row as UnitRelationRow),
    ),
    shareToken,
    shareEnabled,
  };
...
export async function persistUnitRelationDelete(
  relationId: string,
): Promise<void> {
  if (!supabase) return;
...
export async function persistUnitMemberRole(
  memberId: string,
  role: "single" | "partner1" | "partner2",
): Promise<void> {
  if (!supabase) return;
